...
ipAddress::ipAddress(std::string _ip, int idx){
  ...
  ipDecimal = partA*(pow(256,3)) + partB*(pow(256,2)) + partC*(pow(256,1)) + partD;
...
unsigned int ipAddress::getIpDecimal(){
  return ipDecimal;
}
...
std::ostream& operator<<(std::ostream& os, const ipAddress& ip) {
    os << ip.ipString<<" Entradas: "<<ip.degreeIn<<" Salidas: "<<ip.degreeOut<<std::endl;
    return os;
}
